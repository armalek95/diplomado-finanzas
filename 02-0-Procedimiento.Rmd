...
La ecuación (\@ref(eq:fcfeq)) para el cálculo del flujo de caja libre *"Free Cash Flow"* utilizada a lo largo del trabajo fue tomada de [@Berk2017].
...
La ecuación (\@ref(eq:npveq)) utilizada para el Valor Presente Neto es una 
adaptación de la ecuación para el método *"The Present Worth Method"* tomada de [@Sullivan2015].
...
kbl(df, col.names = gsub("[.]", " ", names(df)), 
