...
```{r inputmontecarlo}
# Calcular las ordenes anuales esperadas
ordenes_anuales_mc <- expectativa_anual / 30

formulacion_mc <- "E1"
compuesto_activo_mc <- "Clorhidrato de difenhidramina"
calc_costo_mc <- function(compuesto_activo_mc, tabletas) {
  
  test_mc <- costo_formulacion$compuesto_activo == compuesto_activo_mc
  costo_tableta <- costo_formulacion$costo_tableta[test_mc]
  
  return(costo_tableta * tabletas)
}

```


```{r montecarlotabletas}

ordenes_mc <- ordenes_personalizadas(ordenes_anuales_mc)

costo_ordenes_mc <- ordenes_mc %>%
  # Agregar numero de tabletas a la preescripcion
  mutate(tabletas=round(rnorm(ordenes_anuales_mc, 30, 1),0)) %>%
  # Calcular el costo de los materiales necesarios para cada orden
  mutate(c_materiales=pmap(list(materiales, tabletas), 
                           .f=costo_materiales)) %>%
  unnest(c_materiales) %>%
  # Generar valores aleatorios de tiempo de fabricacion
  mutate(t_fabricacion=round(rnorm(ordenes_anuales_mc, t_fabricacion, 1),0)) %>%
  # Calcular el costo de labor
  mutate(c_labor=pmap(list(tabletas, t_fabricacion), 
                      .f=costo_labor)) %>%
  unnest(c_labor) %>%
  # Generar valores aleatorios para el tiempo de impresion
  mutate(t_impresion=round(rnorm(ordenes_anuales_mc,t_impresion,1), 0),
         # Calcular el costo de electricidad y el consumo electrico
         costo_electricidad=rep(costo_electricidad,ordenes_anuales_mc),
         consumo_energetico=rep(consumo_energetico_impresora, 
                                ordenes_anuales_mc)) %>%
  # Calcular el costo energetico total
  mutate(c_energia=pmap(list(materiales, t_impresion, 
                             costo_electricidad, consumo_energetico, 
                             formulacion),
                        .f=costo_energia)) %>%
  unnest(c_energia) %>%
  # Generar valores aleatorios del tiempo de transporte
  mutate(t_transporte=rnorm(ordenes_anuales_mc,t_transporte,1),
         salario_trabajador=rep(salario_trabajador, ordenes_anuales_mc), 
         # Generar valores aleatorios de la distancia de recorrida por entrega
         distancia=rnorm(ordenes_anuales_mc, distancia, 3), 
         precio_gasolina=rep(precio_gasolina, ordenes_anuales_mc), 
         eficiencia_vehiculo=rep(eficiencia_vehiculo, ordenes_anuales_mc),
         numero_tabletas_viaje=rep(numero_tabletas_viaje, 
                                   ordenes_anuales_mc)) %>%
  # Calcular el tiempo de distribucion
  mutate(c_distribucion=pmap(list(t_transporte, salario_trabajador, 
                                  distancia, precio_gasolina, 
                                  eficiencia_vehiculo,
                                  numero_tabletas_viaje), 
  ...
  select(formulacion, compuesto_activo, tabletas, 
         c_materiales, c_energia, c_labor, c_distribucion) %>%
  # Calcular costo total de produccion de la orden
  mutate(c_total=(c_materiales+c_energia+c_labor+c_distribucion)*tabletas)

costo_ordenes_mc %>% 
  group_by(formulacion, compuesto_activo) %>% 
  summarise(tabletas = round(mean(tabletas), 0),
            c_total = round(mean(c_total), 2), .groups = "keep") %>%
  as.data.frame() %>% 
  mutate(c_total = currency(c_total, "$", 2)) %>% 
  kbl(col.names = c("Formulación","Compuesto Activo", "Tabletas", 
                    "Costo Promedio"),
      align = "llcc",
      caption = "Ordenes Promedio Monte Carlo", 
      label = "tabordenesmontecarlos")
