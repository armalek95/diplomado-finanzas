...
```{r inputscasobase}
expectativa_anual <- 200000 #[tabletas/periodo]
markup <- 0.5
...
input_caso_base <- tibble(
  Variable=c("Expectativa de venta de tabletas", "Markup", 
             "Material de Trabajo", "Tasa de Descuento", "Tasa Gravable"),
...
for (a in 1:2) {
  ...
  for (i in 1:3) {
    ...
    for (j in 1:3) {
      ...
      costo_anual_emv <- c(0, rep(escenarios_costo[i]*escenarios_tabletas[j],
                                   10))
      # Agregar Markup
      for (k in 1:3) {
        ...
        for (l in 1:3) {
          ...
          costo_anual_emv <- costo_anual_emv + mant_anual_emv
...
comprar_print %>% 
  ...
  kbl(longtable=T, booktabs=T, 
      col.names = c("Decisión", "Costo", "Volumen", "Markup", 
                    "Mantenimiento", "EMV"),
      align = "lllllr",
      caption = "VME Positivo (Top 20) - Tabletas Personalizables", 
...
```{r graficaemv, fig.cap="Histograma del EMV esperado decisión"}
...
PlotDecisionTree(comprar$CO$CA$TA)
...
sensitivity(variables = sens_results$variable[1:n], 
            high = sens_results$diff[1:n],
            low = sens_results$diff[(n+1):(n*2)]) %>% 
  tornado() +
  labs(
